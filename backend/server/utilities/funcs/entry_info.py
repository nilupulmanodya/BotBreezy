...
def entry_info(utterance,mycursor,db_connection):
	
	#updating session 
	#function consists one entity COU
	print('begining entry info',session['ses_validate'])
	...
	if len(entities)>0:
		for i in range(0, len(entities)):
			...
			#entities needs to entry_info (bio medical engineering,computer science)
		...
		if len(ent_COU_ls)==1:
			...
			response = {'fun_res':{
					'is_completed':False,
					'content':'Tell me which course entry requirements do you need ?',
					'is_option':False,
					'is_entity':True,
					'options':None,
